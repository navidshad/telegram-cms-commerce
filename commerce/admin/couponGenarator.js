...
var showGenerator = async function(userid, name)
{
    ...
    var fn_createTime   = qt['commerce'] + '-' + qt['admin'] + '-' + qt['generator'] + '-' + qt['createtime'] + '-' + gen.id;
    ...
    {        
        detailArr = [ 
            ...
            {'text': 'زمان صدور', 'callback_data': fn_createTime}],
...
{
    var mode = 'buy';
    var createtime = 'affter';
    var generators = await global.fn.db.generator.find({'mode': mode, 'createtime': createtime, 'status': true}).exec().then();
